...
We will see a quick setup to configure the ``changedetect`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``changedetect`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``changedetect`` formulas under ``/srv/salt/changedetect/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/changedetect/parameters/dns:domain/
    ln -s dns:domain /srv/salt/changedetect/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/changedetect/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/changedetect/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``changedetect`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``changedetect`` formulas under ``/srv/salt/changedetect/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/changedetect/parameters/roles
...
We create a configuration for the role ``changedetect/server`` in ``/srv/salt/changedetect/parameters/roles/changedetect/server.yaml``:
...
We create another configuration for the role ``changedetect/client`` in ``/srv/salt/changedetect/parameters/roles/changedetect/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles changedetect/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles changedetect/client
...
.. code-block:: console
    ...
    mv /srv/salt/changedetect/parameters/dns:domain/  '/srv/salt/changedetect/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-changedetect-as-json:
      test.nop:
        - name: {{ changedetect | json }}
...
Here is an example based on `template-formula/changedetect/config/file.sls`_:
...
.. code-block:: sls
    ...
    changedetect-config-file-file-managed:
      file.managed:
        - name: {{ changedetect.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='changedetect-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ changedetect.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``changedetect`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ changedetect['winner'] }}
...
.. _template-formula/changedetect/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/changedetect/config/file.sls
