...
{%- if      changedetect._podman_compose
    and not changedetect.installation.podman.compose_systemd
    and     changedetect.installation.podman.rootless %}
...
Selenium Chrome environment file is managed:
...
{%- if changedetect.selenium.enable %}
    - source: {{ files_switch(['selenium-chrome.env', 'selenium-chrome.env.j2'],
...
Playwright Chrome environment file is managed:
...
{%- if changedetect.playwright.enable %}
    - source: {{ files_switch(['playwright-chrome.env', 'playwright-chrome.env.j2'],
...
{%- if changedetect._podman_compose %}
{%-   if changedetect.installation.podman.compose_systemd %}
...
{%-     if changedetect.installation.podman.rootless %}
    - runas: {{ changedetect.lookup.user.name }}
    - require:
      - user: {{ changedetect.lookup.user.name }}
{%-     endif %}
{%-   elif changedetect.installation.podman.compose_pods %}
...
{%-   if not changedetect.installation.podman.compose_systemd %}
